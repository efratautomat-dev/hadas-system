// The supplier ledger engine — Deno-side copy of `src/lib/ledgerEngine.ts`.
//
// ⚠️ THE TWO FILES MUST BE CHANGED TOGETHER, exactly like the `vat.ts` pair next
// to it. They cannot be one file: the frontend is bundled by Vite from `src/`,
// the edge functions run on Deno, and `.vercelignore` keeps `supabase/` out of
// the frontend build — neither side can import the other.
//
// Unlike `vat.ts`, this pair is not maintained by hand-eye: `scripts/check-twins.mjs`
// compares everything below this header comment and fails `npm run lint` the
// moment the two diverge. Copy the whole file; do not patch one side.
//
