...
## 12. קליטת סחורה — לחשוב מחדש על חוויית העובדת (בעלת העסק, 14.09.2026)

> **פתוח לאפיון. לא באג — הצטברות.**

בעלת העסק, אחרי שבדקה את הסבב על נתוני אמת: *"אני רוצה לחשוב מחדש על חוויית
המשתמש בקליטת סחורה. צריך להסביר לעובדות בשני משפטים מה לעשות, שלא לסבך אותן
ושלא יטיילו במלא מקומות."*

**מה שקרה כאן, ולמה זה הצטבר ולא נשבר:** כל דלת נוספה בנפרד ובצדק שלה —

| דלת | למה נבנתה |
|---|---|
| צילום מסמך (מסך נפרד) | המסך הראשון שהיה, לפני הפייפליין |
| קליטת סחורה ← צילום | "דלת אחת, שתי דרכים פנימה" |
| קליטת סחורה ← דף בכתב יד | הטופס המודפס, החלטת הבעלים |
| קליטת סחורה ← הקלדה | כשאין תעודה בכלל |
| בתוך דף הסחורה (מקופל) | "העבודה ממשיכה מהשורה" |
| "הסחורה הגיעה עם החשבונית" | לא לבקש להקליד מסמך שכבר מוצמד |
| "הגיע" על כרטיס הזמנה | ההזמנה היא רגל בשרשרת |

שבע נקודות כניסה לפעולה אחת — *"הגיעה סחורה"* — וכל אחת מהן נכונה בפני עצמה.
**זו בדיוק הצורה שנוצרת כשמוסיפים נכון שבע פעמים ברצף ואף פעם לא מסתכלים על
השלם.** העובדת לא צריכה ללמוד מפה; היא צריכה לדעת מה לעשות כשיש משטח מולה.

**המבחן שהאפיון צריך לעמוד בו — שני משפטים.** אם אי אפשר להסביר את קליטת הסחורה
בשני משפטים לעובדת חדשה, האפיון לא נגמר. נוסח מוצע כנקודת מוצא:

> "יש סחורה? פתחי את הספק ולחצי **קליטת סחורה**. אם הגיע מסמך — צלמי אותו; אם
> לא — רשמי מה הגיע."

**שאלות פתוחות שהאפיון צריך להכריע:**

1. **מסך "צילום מסמך" הנפרד — האם הוא עוד צריך להתקיים?** היום הוא הדרך היחידה
   לצלם חשבונית או זיכוי, אבל הוא גם דרך שנייה לצלם תעודת משלוח — והיא זו
   שפותחת שורה משלה במקום להיצמד. ⚠️ **לפני שמוחקים אותו**: הוא הכניסה היחידה
   לחשבונית ולזיכוי.
2. **האם "קליטת סחורה" צריכה להיפתח מהספק בלבד?** הספק הוא השדה היחיד שיכול
   לתייק סחורה לאדם הלא נכון, והוא כבר ידוע כשנכנסים מכרטיס. פתיחה ממסך הסחורה
   מחזירה את בורר הספק — ואת הכפתור האפור שאיש לא הבין (ראו `spec/STATUS.md`,
   הלקח על כפתור מנוטרל).
3. **האם שלוש הדלתות הן בכלל שאלה לעובדת?** "צילום / דף בכתב יד / הקלדה" הן
   תיאור של **מה יש לה ביד**, לא בחירה. ייתכן ששאלה אחת — "יש מסמך?" — מספיקה,
   והמערכת מחליטה מה להפעיל.
4. **מה קורה כשהחשבונית כבר במערכת?** היום זה כפתור נוסף על השורה. אולי זו
   בכלל התשובה הראשונה שצריך להציע, כי זה המצב הנפוץ ביותר בחנות.

**לא לפצל את זה לתיקונים.** כל אחד משבעת הפריטים בטבלה נוסף כתיקון נכון, וזה מה
שהביא לכאן. הצעד הבא הוא **סקיצה אחת של הזרימה כולה** לאישור הבעלים — כמו שנעשה
בפייפליין עצמו — ורק אחריה קוד.
